...
| ⚙️ 配置编辑 | 路径列表（可拖拽排序）、取消轮询、排除字段、水印 PNG |
| 🔍 预检测 | 统计合规视频数量与分组，异常路径整行高亮提示 |
| ▶️ 批量成片 | 调用 PowerShell 脚本批量生成，实时互斥锁、进度条与日志 |
| 🗂️ 任务列表 | 排队任务可暂停 / 继续 / 停止，失败保留完整输出并红字提示原因 |
| 🎛️ 复刻模式 | 完全复刻 / 去重复刻两种模式，日志全局搜索 |
| ⌨️ 索引与排序 | A-Z 拼音索引条、名称 / 时间排序切换、皮肤切换 |
| 📐 灵活布局 | 侧栏 / 预览区宽度均可拖拽调整 |
